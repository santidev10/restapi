...
class SegmentListGenerator(object):
    ...
    def _process(self, batch):
        ...
        :return:
        """
        sorted_by_category_whitelist = self._sort_by_category(batch)
    ...
    def _truncate_segment(self, segment, size):
        """
        ...
        """
        sort_key = self.RELATED_SEGMENT_SORT_KEY
        search_query = QueryBuilder.build().must().terms().field(SEGMENTS_UUID_FIELD).value(segment.uuid).get()
        docs_to_remove = self.es_manager.search(search_query, sort=sort_key, offset=size)
        ...
        self.es_manager.remove_from_segment(remove_query, segment.uuid)
