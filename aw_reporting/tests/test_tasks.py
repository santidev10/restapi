...
from aw_reporting.models import AWConnection, AWConnectionToUserRelation, \
    AWAccountPermission, Account, Ad, AdGroupStatistic, AdGroup, Campaign
...
class GetAdsTestCase(testcases.TestCase):
    ...
    def setUpTestData(cls):
        super(GetAdsTestCase, cls).setUpTestData()
        user = UserProfile.objects.create(email='test@test.com')
        account = Account.objects.create(id='7155851537', name='Hall9000',
                                         can_manage_clients=True)
        cls.connection = AWConnection.objects.create(
            email="anna.chumak1409@gmail.com",
            refresh_token='1/MJsHAtsAl1YYus3lMX0Tr_oCFGzHbZn7oupW-2SyAcs')
        AWAccountPermission.objects.create(aw_connection=cls.connection,
    ...
    def setUp(self):
        super(GetAdsTestCase, self).setUp()
