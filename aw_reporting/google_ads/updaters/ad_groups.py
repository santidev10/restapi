        ...
        for row in ad_group_performance:
            ...
            if ad_group_id not in updated_ad_groups:
                ...
                ad_group_data = {
                    ...
                    "cpv_bid": float(row.ad_group.cpv_bid_micros.value) / 10 ** 6 if row.ad_group.cpv_bid_micros else None,
                    "cpm_bid": float(row.ad_group.cpm_bid_micros.value) / 10 ** 6 if row.ad_group.cpm_bid_micros else None,
                    "cpc_bid": float(row.ad_group.cpc_bid_micros.value) / 10 ** 6 if row.ad_group.cpc_bid_micros else None,
                }
