...
class PacingReportOpportunityBufferTestCase(APITestCase):
    ...
    def test_success_response_cpm_with_goal_factor(self):
        now = timezone.now()
        opportunity = Opportunity.objects.create(
        ...
        url = reverse("aw_reporting_urls:pacing_report_opportunity_buffer",
                      args=(opportunity.id,))
        response = self.client.put(url, json.dumps(update), content_type="application/json")
        cpm_buffer = self.pacing_report.goal_factor + (update["cpm_buffer"] / 100)
    ...
    def test_success_response_cpm_with_big_goal_factor(self):
        now = timezone.now()
        opportunity = Opportunity.objects.create(
            id="1", name="opportunity", start=now - timedelta(days=1),
        ...
        url = reverse("aw_reporting_urls:pacing_report_opportunity_buffer",
                      args=(opportunity.id,))
        response = self.client.put(url, json.dumps(update), content_type="application/json")
        cpm_buffer = self.pacing_report.big_goal_factor + (update["cpm_buffer"] / 100)
        expected_plan_impressions = flight.ordered_units * cpm_buffer
    ...
    def test_success_response_cpv_with_goal_factor(self):
        ...
        url = reverse("aw_reporting_urls:pacing_report_opportunity_buffer",
                      args=(opportunity.id,))
        response = self.client.put(url, json.dumps(update), content_type="application/json")
        cpv_buffer = self.pacing_report.goal_factor + (update["cpv_buffer"] / 100)
    ...
    def test_success_response_cpv_with_big_goal_factor(self):
        now = timezone.now()
        opportunity = Opportunity.objects.create(
            id="1", name="opportunity", start=now - timedelta(days=1),
            end=now + timedelta(days=1), probability=100, budget=1000000)
        ...
        url = reverse("aw_reporting_urls:pacing_report_opportunity_buffer",
                      args=(opportunity.id,))
        response = self.client.put(url, json.dumps(update), content_type="application/json")
        cpv_buffer = self.pacing_report.big_goal_factor + (update["cpv_buffer"] / 100)
        expected_video_views = flight.ordered_units * cpv_buffer
    ...
    def test_success_response_cpv_cpm_with_big_goal_factor(self):
        now = timezone.now()
        opportunity = Opportunity.objects.create(
            id="1", name="opportunity", start=now - timedelta(days=1),
            end=now + timedelta(days=1), probability=100, budget=1000000)
        ...
        flight_cpm = Flight.objects.create(
            id="1", placement=placement_cpm, name="flight_cpm",
            start=now - timedelta(days=1), end=now + timedelta(days=1), ordered_units=100
        )
        flight_cpv = Flight.objects.create(
            id="2", placement=placement_cpv, name="flight_cpv",
            start=now - timedelta(days=1), end=now + timedelta(days=1), ordered_units=100
        )
        update = dict(
            cpm_buffer=20,
