...
class Command(BaseCommand):
    ...
    def process_export(self):
        ...
        if audit_type == 2:
            file_name = export_funcs.export_channels(self.audit, self.audit.id, clean=self.export.clean)[0]
            count = AuditChannelProcessor.objects.filter(audit=self.audit)
        else:
            file_name = export_funcs.export_videos(self.audit, self.audit.id, clean=self.export.clean)[0]
