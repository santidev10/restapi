...
class ChannelTrackApiView(APIView, PermissionRequiredMixin):
    ...
    def post(self, request, *args, **kwargs):
        channel_ids_file = request.data['channel_ids'] if "channel_ids" in request.data else None
        if not channel_ids_file:
            raise ValidationError("channel_ids file is required.")
        channel_ids_file_split = channel_ids_file.name.split(".")
        if len(channel_ids_file_split) != 2:
            raise ValidationError(f"Invalid channel_ids file. Expected CSV file. Received {channel_ids_file}.")
        channel_ids_file_type = channel_ids_file_split[1]
        if channel_ids_file_type != "csv":
            raise ValidationError(f"Invalid channel_ids file type. Expected CSV. Received '{channel_ids_file_type}'.")
        with open(channel_ids_file.name, "r") as f:
            reader = csv.reader(f)
            new_channel_ids = []
            for row in reader:
