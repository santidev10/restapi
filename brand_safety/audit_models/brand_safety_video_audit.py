...
class BrandSafetyVideoAudit(object):
    ...
    def __init__(self, video, audit_utils, ignore_blacklist_data=False):
        self.audit_utils = audit_utils
        self.score_mapping = audit_utils.score_mapping
        self.default_category_scores = audit_utils.default_full_score
        self.language_processors = audit_utils.bad_word_processors_by_language
    ...
    def _run_brand_safety_audit(self):
        ...
        try:
            ...
            transcript_processor = keyword_processor
        tag_hits = self.audit_utils.audit(self.video.tags, constants.TAGS, keyword_processor)
        title_hits = self.audit_utils.audit(self.video.general_data.title, constants.TITLE, keyword_processor)
        description_hits = self.audit_utils.audit(self.video.general_data.description, constants.DESCRIPTION, keyword_processor)
        transcript_hits = self.audit_utils.audit(self.video.transcript, constants.TRANSCRIPT, transcript_processor)
        all_hits = tag_hits + title_hits + description_hits + transcript_hits

        # Calculate Universal keywords hits, if not all processor
        if universal_processor:
            universal_tag_hits = self.audit_utils.audit(self.video.tags, constants.TAGS,
                                                        universal_processor)
            universal_title_hits = self.audit_utils.audit(self.video.general_data.title, constants.TITLE,
                                                          universal_processor)
            universal_description_hits = self.audit_utils.audit(self.video.general_data.description,
                                                                constants.DESCRIPTION,
                                                                universal_processor)
            universal_transcript_hits = self.audit_utils.audit(self.video.transcript,
                                                               constants.TRANSCRIPT,
                                                               universal_processor)
            all_hits += universal_tag_hits + universal_title_hits + universal_description_hits + \
                        universal_transcript_hits
    ...
    def calculate_brand_safety_score(self, *hits):
        ...
        :return: tuple -> (int) total score, (dict) scores by category
        """
        brand_safety_score = BrandSafetyVideoScore(self.video.main.id, self.default_category_scores)
        for word in hits:
            multiplier = self.score_multiplier.get(word.location, 1)
            try:
                keyword_category = self.score_mapping[word.name]["category"]
        ...
        # If blacklist data available, then set overall score and blacklisted category score to 0
        if self.ignore_blacklist_data is False:
            for category_id in self.video.task_us_data.brand_safety:
                if category_id in brand_safety_score.category_scores:
                    brand_safety_score.category_scores[category_id] = 0
                    if category_id not in BadWordCategory.EXCLUDED:
                        brand_safety_score.overall_score = 0
    ...
    def instantiate_es(self):
        video = Video(self.video.main.id)
        """
        Instantiate Elasticsearch video model with brand safety data
        ...
        brand_safety_data = {
            "overall_score": brand_safety_score.overall_score if brand_safety_score.overall_score >= 0 else 0,
            "transcript_language": self.video.transcript_language,
            "categories": {
        ...
        video.brand_safety = brand_safety_data
        video.populate_channel(id=self.video.channel.id, title=self.video.channel.title)
        return video
