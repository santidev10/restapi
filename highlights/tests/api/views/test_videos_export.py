...
class HighlightVideoExportApiViewTestCase(ExtendedAPITestCase, ESTestCase):
    ...
    @mock_s3
    @mock.patch("highlights.api.views.videos_export.HighlightVideosExportApiView.generate_report_hash",
    ...
    def test_export_by_ids(self, *args):
        ...
        self._request_collect_file(**{"main.id": videos[0].main.id})
        response = self._request_export()
    ...
    def test_export_by_ids_deprecated(self, *args):
        ...
        self._request_collect_file(**{"ids": videos[0].main.id})
        response = self._request_export()
...
def get_collect_file_url(**kwargs):
    return reverse(HighlightsNames.VIDEOS_PREPARE_EXPORT, [Namespace.HIGHLIGHTS], query_params=kwargs or None)


def get_export_url(export_name):
    return reverse(HighlightsNames.VIDEOS_EXPORT, [Namespace.HIGHLIGHTS], args=(export_name,),)
