...
class ContexualAnalyzerTestCase(ExtendedAPITestCase):
    ...
    def test_not_analyzed_no_threshold(self):
        ...
        self.assertEqual(results["content_categories"]["matched"], 0)
        # However occurrences should still be counted
        self.assertTrue("Music" in results["content_categories"]["top_occurrence"])
    ...
    def test_matched_categories(self):
        ...
        for analysis in analyses:
            analyzer.analyze(analysis)
        results = analyzer.get_results()
        self.assertEqual(results["content_categories"]["matched"], round(2 / 3 * 100, 2))
        self.assertEqual(results["content_categories"]["top_occurrence"][0], "Music")
        self.assertEqual(set(results["content_categories"]["top_occurrence"]),
                         {*params["content_categories"], *["Cars"]})
