...
@celery_app.task
def update_mcc_campaigns_task(mcc_id: int, ouath_account_id: int):
    """
    Update campaigns for MCC account
    :param mcc_id: Google Ads MCC account id
    :param ouath_account_id: OAuthAccount pk
    :return:
    """
    ouath_account = OAuthAccount.objects.get(id=ouath_account_id)
...
@celery_app.task
def update_cid_campaigns_task(google_ads_cid: int, ouath_account_id: int, historical=False):
    """
    Update single Google Ads CID account
    :param google_ads_cid: Google Ads CID
    :param ouath_account_id: OAuthAccount pk
    ...
    update_campaigns(google_ads_cid, ouath_account, date_range=date_range, predicates=predicates)


def update_campaigns(google_ads_cid: int, ouath_account: OAuthAccount, predicates=None, date_range=None) -> None:
    """
    ...
    :param
    """
    client = get_client(client_customer_id=google_ads_cid, refresh_token=ouath_account.refresh_token)
    ...
    for item in to_update + to_create:
        item.account_id = ouath_account.id
